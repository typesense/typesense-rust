...
pub fn typesense_collection_derive(input: TokenStream) -> TokenStream {
    let item: ItemStruct =
        syn::parse(input).expect("Document can be only be derived for structs");
...
fn impl_typesense_collection(item: ItemStruct) -> syn::Result<TokenStream> {
    ...
    let collection_name =
        extract_collection_name(&item.attrs)?.unwrap_or(name.to_string().to_lowercase());
...
fn extract_default_sorting_field(attributes: &[Attribute]) -> syn::Result<Option<String>> {
    let default_sorting_field = attributes.iter().filter_map(|attr| {
                       ...
                       if  i == "default_sorting_field" {
                        i.clone()
                           ...
                           return None;
                            ...
                            return Some(Err(syn::Error::new(i.span(), "expected: typesense(default_sorting_field  = \"...\")")));
                       }
                    },
                    Some(tt) => {
                        return Some(Err(syn::Error::new_spanned(&tt, format!("Unexpected {}", tt))));
                },
                None => {return Some(Err(syn::Error::new_spanned(&attr, "expected: typesense(default_sorting_field  = \"...\")")));}
            };
                let eq_sign =  match tokens.next() {
                    Some(proc_macro2::TokenTree::Punct(ref p)) => {assert_eq!(p.as_char(), '='); p.clone()},
                    Some(tt) =>  {
                        return Some(Err(syn::Error::new_spanned(&tt, format!("Unexpected {}, expected equal sign(=)", tt))));
                    },
                    None => {
                        return Some(Err(syn::Error::new_spanned(&attribute_tt, "expected: equal sign(=)")));
                    }
                };
                let default_sorting_field = {
                    let lit =
                    match tokens.next(){
                    Some(proc_macro2::TokenTree::Literal( ref i)) => syn::Lit::new(i.clone())
                    ,
                    Some(tt) =>  {
                        return Some(Err(syn::Error::new_spanned(&tt, format!("Expected string literal, do you mean \"{}\"?", tt))));
                },
                _ =>     return Some(Err(syn::Error::new_spanned(&eq_sign, "Expected string literal"))),
                };
                if  let syn::Lit::Str(s) = lit {
                    s.value()
                }else{
                        return Some(Err(syn::Error::new_spanned(lit, "default_sorting_field must be equal to a literal string")));                
                }
            };
